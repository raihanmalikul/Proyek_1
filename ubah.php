...
$result = mysqli_query($conn,"SELECT * FROM laundry_pakaian WHERE id_pakaian = '$id' ");
...
<input type="hidden" name="id" value="<?= $ord["id_pakaian"]; ?>">

        <h4 class=" text-center font-weight-bold pt-5">Menu ubah status</h4>
          <hr color="#0263A0">
          <div class="from-group pt-2">

          <div class="from-group pt-2">
                <label>Berat pakaian</label>
                <input type="text" name="berat_pakaian" class="form-control" placeholder="Masukan berat pakaian (Kg)" value="<?= $ord['berat_pakaian']; ?>"  required >
                <small id="emailHelp" class="form-text text-muted">catatan : catatan : Input Berat Pakaian pada laundry ini menggunakan sistem pembulatan kebawah untuk (1-1,5) kg menjadi 1 kg dan untuk (1,6-2) kg dibulatkan keatas menjadi 2 kg.</small>
            </div>
            ...
            <div class="from-group pt-2">
            <label>Biaya laundry pakaian</label>
            <?php $berat = 0;?>
            <?php 
            if ( isset($_POST["berat_pakaian"])>0) {
              $berat = $_POST["berat_pakaian"]*6000;}?> 
              <input  readonly type='text' name='total_biaya_pakaian' class='form-control'  value="<?= $berat;?>">
            </div>
            ...
            <div class="from-group pt-2">
                ...
                <option value="<?= $ord['status_pembayaran']; ?>"> <?= $ord['status_pembayaran']; ?></option>
                 ...
                 <option value="Sudah di Bayar">Sudah dibayar</option>      
                </select>
            </div>
            <div class="from-group pt-2">
